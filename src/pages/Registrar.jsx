...
const Registrar = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if([nombre, email, password, repetirPassword].includes("")) return setAlerta({msg: 'Hay campos vacíos', error: true})
    if(password !== repetirPassword) return setAlerta({msg: 'Los passwords no son iguales', error: true})
    if(password.length < 6) return setAlerta({msg: 'El password es muy corto. Mínimo 6 caracteres', error: true})
